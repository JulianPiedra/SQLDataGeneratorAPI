...
namespace SqlDataGenerator.Logic
{
    public class EmailGeneration : IEmailGeneration
    {
        ...
        public async Task<BusinessLogicResponse> GenerateEmail(int? records)
        {
            ...
            try
            {
                ...
                var randomNames = await FetchFromDatabase.FetchStringListFromDatabase<FirstName>(
                            records,
                            Context.FirstName,
                            f => f.FirstName1);
                ...
                var randomEmailExtension = await FetchFromDatabase.FetchStringListFromDatabase<Email>(
                            records,
                            Context.Email,
                            f => f.EmailExtension);
